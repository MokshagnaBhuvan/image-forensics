...
# Build 2D Convolutional Autoencoder with corrected layers
# Build 2D Convolutional Autoencoder with corrected layers
def build_autoencoder(input_shape):
    input_img = Input(shape=input_shape)
    
    # Encoder
    x = Conv2D(32, (3, 3), padding='same', strides=2)(input_img)  # Downsample to 64x64
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = Dropout(0.2)(x)  # Dropout layer

    x = Conv2D(64, (3, 3), padding='same', strides=2)(x)  # Downsample to 32x32
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = Dropout(0.3)(x)  # Dropout layer

    x = Conv2D(128, (3, 3), padding='same', strides=2)(x)  # Downsample to 16x16
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = Dropout(0.3)(x)  # Dropout layer

    x = Conv2D(256, (3, 3), padding='same', strides=2)(x)  # Downsample to 8x8
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.2)(x)
    x = Dropout(0.3)(x)  # Dropout layer

    encoded = Conv2D(512, (3, 3), padding='same', strides=2)(x)  # Downsample to 4x4

    # Decoder (Correct upsampling layers to match original image size)
    ...
    # Final convolution to output the original image size (128x128)
...
# Save the model
autoencoder.save("autoencoder_model_robust2.h5")
